...
public class BrowseListingsServlet extends HttpServlet {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(BrowseListingsServlet.class);
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try (Connection conn = DBConnection.getConnection();
        PreparedStatement stmt = conn
            .prepareStatement("SELECT * FROM listings WHERE user_id != ? AND status != 'exchanged'")) {
      ...
      response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "An error occurred while retrieving listings.");
    }
  }
}